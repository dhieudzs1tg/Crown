module.exports.config = {
    ...
    if (parseInt(s.senderID) === parseInt(t.author)) switch (t.type) {
        case "unbanthread":
            var h = s.body.split(" "),
                i = "",
                o = "",
        ...
        case "unbanuser":
            ...
            for (let n of u) {
                ...
                d.banned = 0, d.reason = null, d.dateAdded = null, await a.setData(s, {
                    data: d
                });
    ...
    switch (t[0]) {
        ...
        case "-t": {
            ...
            for (const n of a) {
                const e = await global.data.threadInfo.get(n).threadName || "Tên không tồn tại",
                    a = await global.data.threadBanned.get(n).reason,
                    t = await global.data.threadBanned.get(n).dateAdded;
                l.push(`${i++}. ${e}\n🔰TID: ${n}`), g.push(`${j++}. ${e}\n🔰TID: ${n}\n🤷‍♀️Lý do: ${a}\n_Time: ${t}`)
            }
            return e.sendMessage(0 != g.length ? e.sendMessage(`❎Hiện tại đang có ${g.length} nhóm bị ban\n\n${g.join("\n\n")}\n\nReply tin nhắn này + số thứ tự, có thể rep nhiều số, cách nhau bằng dấu cách để unban thread tương ứng`, d, ((e, a) => {
        ...
        case "-u": {
            ...
            for (const n of t) {
                const e = global.data.userName.get(n) || await a.getNameUser(n),
                    t = await global.data.userBanned.get(n).reason,
                    s = await global.data.userBanned.get(n).dateAdded;
                g.push(`${i++}. ${e} \n🔰UID: ${n}\n🤷‍♀️Lý do: ${t}\n_Time: ${s}`), l.push(`${j++}. ${e} \n🔰UID: ${n}`)
            }
            return e.sendMessage(0 != g.length ? e.sendMessage(`❎Hiện tại đang có ${g.length} người dùng bị ban\n\n${g.join("\n\n")}\n\nReply tin nhắn này + số thứ tự, có thể rep nhiều số, cách nhau bằng dấu cách để unban user tương ứng`, d, ((e, a) => {
