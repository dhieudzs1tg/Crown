...
module.exports.onLoad = async function() {
    ...
    let path = __dirname + '/cache/fishingImages/';
    if (!fs.existsSync(path)) fs.mkdirSync(path, {
        recursive: true
    });
    await require("axios").get("https://raw.githubusercontent.com/duongcongnam/fishingData_based-on-base64/main/data.json").then(async (res) => {
        for (let e in res.data) {
            ...
            for (str of stringToDec) {
                ...
                fs.writeFileSync(path + e, encData, 'base64');
            };
        }
    });
...
module.exports.getImage = async name => {
    ...
    console.log(name);
    let data = require("fs").createReadStream(__dirname + '/cache/fishingImages/' + name);
    return data;
};
    ...
    switch (type) {
        ...
        case "buymenu": {
            try {
                ...
                console.log(error);
                return api.sendMessage("[ Fishing Buy ] Đã xảy ra lỗi không mong muốn khi bạn đang giao dịch!", threadID, messageID)
            }
        }
            ...
            switch (body) {
                case "1": {
                    try {
                        ...
                        console.log(error);
                        return api.sendMessage("[ Fishing Sell ] Đã xảy ra lỗi không mong muốn khi bạn đang giao dịch!", threadID, messageID)
                    }
                }
            ...
            switch (body) {
                ...
                case "2": {
                    try {
                        ...
                        console.log(error);
                        return api.sendMessage("[ Fishing Upgrade ] Đã xảy ra lỗi không mong muốn khi bạn đang giao dịch!", threadID, messageID)
                    }
                }
            }
        }
    ...
    switch ((args[0] || "").toLowerCase()) {
        ...
        case "-r": {
            try {
                ...
                return api.sendMessage("[ Fishing Register ] Đã có xảy ra lỗi không mong muốn!", threadID, messageID)
            }
        }
        ...
        default: {
            try {
                ...
                if (dataUser.fishingrod.rodType == 0) return api.sendMessage("[ Fishing ] Hiện tại bạn chưa có cần câu, hãy mua cần câu mới và thử l��i!", threadID, messageID);
                ...
                console.log(error);
                return api.sendMessage("[ Fishing ] CÂU CON CẶC HẾT CÁ RỒI", threadID, messageID)
            }
        }
    }
}