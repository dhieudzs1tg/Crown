module.exports.config = {
    ...
    usages: "[reason] nhấp 2 lần ko cần prefix 'check' để xem những ng đã tag",
    cooldowns: 5
};
...
const moment = require("moment-timezone");
  var tg = moment.tz("Asia/Ho_Chi_Minh").format("HH:mm:ss || D/MM/YYYY");
    ...
    if(a.length !== 0) {
        ...
        for (let i of a) {
            ...
            if(g == true) {
                ...
                w.sendMessage(`❗️ Người dùng ${(await Users.getData(i)).name} đang bận với lí do: ${s.r}`, t, n)
                s.v.push({
                    c: c,
                    y: y + "\n⏱ Vào lúc: " + tg
                })
    ...
    if(y == "Check" || y == "Alo bot" || y == "Check tag") {
        if(x == true && u.p == 0) {
            var m = `✌️ Chào mừng bạn quay trở lại!\n`
