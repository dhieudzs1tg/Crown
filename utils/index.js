...
module.exports.throwError = async(command, threadID, messageID) => {
  const cache = __dirname + "/../modules/commands/cache/1.jpg"
	const threadSetting = global.data.threadData.get(threadID) || {};
  const prefix = threadSetting.PREFIX || global.config.PREFIX;
  var get = (await axios.get(global.config.configSetupAPI.imgLimk_nino)).data
   global.client.api.sendMessage({body: global.getText("utils", "throwError", prefix, command), attachment: await DownLoad(get.url, cache)}, threadID, () => fs.unlinkSync(cache), messageID)
   }
...
await download.image({ url: link, dest: path })
